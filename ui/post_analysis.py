...
import os
import logging
from PySide6.QtWidgets import QMessageBox, QWidget, QVBoxLayout, QLabel, QPushButton, QLineEdit, QTableWidget, QTableWidgetItem, QFileDialog, QHBoxLayout, QGridLayout, QComboBox, QProgressBar
...
class PostAnalysisWidget(QWidget):
    def __init__(self):
        super().__init__()
        ...
        self.post_link_input = QLineEdit()
        self.post_link_input.setPlaceholderText("Введите ссылку на пост ВКонтакте (например, https://vk.com/wall-1_12345)...")
        layout.addWidget(self.post_link_input)
    ...
    def clear_data(self):
        logger.info("Очистка данных в PostAnalysisWidget")
        self.post_link_input.clear()
    ...
    def load_comments(self):
        post_url = self.post_link_input.text().strip()
        if not post_url:
            logger.warning("Пустая ссылка на пост")
            self.comments_table.setRowCount(0)
            return
    ...
    def get_comments(self, post_url):
        try:
            ...
            return self.vk_client.get_comments(post_url, max_count=100)
